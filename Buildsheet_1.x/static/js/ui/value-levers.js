...
function fetchTables(current) {
	$(current).parent().find('.lever-tables').html('');
	$.ajax({
			'type'	:	'post',
			'url'	:	'/valueLevers/',
			'async': false,
		...
		if (res.status =='2'){
			...
			$.each(res.columns, function( index, value){
				...
				value = value[0]
				if(index==0){
				col_row +='<div style="margin-top: -25px;height: 66px;"> <svg id="chart" style="width: 21px;margin: -8px;"> <line x1="20" y1="40" x2="20" y2="110" style="stroke: #555555;stroke-width:1;"></line> </svg> <svg class="svg-link" style="margin-left: 4px;"><path stroke="black" d="M5 40 l215 0"></path></svg> <button type="button" level="'+$(current).attr('level')+'" class="btn btn-primary custom-btn column-button table-button" table-name="'+value+'" style="background: '+bg+';">'+value.replace(/^\./g,'')+'</button><div style="display: grid;width: max-content;padding: 0;margin: 0;position: absolute;margin-left: 200px;margin-top: -69px;" class="lever-columns" hidden="true"></div></div></div>';

				}
				else{
				col_row +='<div style="margin-top: 0px;height: 66px;"> <svg id="chart" style="width: 21px;margin: -8px;"> <line x1="20" y1="40" x2="20" y2="110" style="stroke: #555555;stroke-width:1;"></line> </svg> <svg class="svg-link" style="margin-left: 4px;"><path stroke="black" d="M5 40 l215 0"></path></svg> <button level="'+$(current).attr('level')+'" type="button" class="btn btn-primary custom-btn column-button table-button" table-name="'+value+'" style="background: '+bg+';">'+value.replace(/^\./g,'')+'</button><div style="display: grid;width: max-content;padding: 0;margin: 0;position: absolute;margin-left: 200px;margin-top: -69px;" class="lever-columns" hidden="true"></div></div></div>';
				}
			});
			...
			$('.table-button').on('click', function(e) {
				...
				val = $(this).parent().find('.lever-columns').prop('hidden');
				// console.log($(this).parent().parent().find);
				$(this).parent().parent().find('.lever-columns').prop('hidden','true');
				$(this).parent().find('.lever-columns').prop('hidden',val);
...
function fetchColumns(current,table) {
	$(table).parent().parent().find('.lever-columns').html('');
	$.ajax({
			'type'	:	'post',
			'url'	:	'/valueLevers/',
			'async': false,
			'data'	:	{'lever_name':'Reduce SNF cost','level':$(table).attr('level'),'table_name':$(table).attr('table-name') , 'action': 'fetchColumns','csrfmiddlewaretoken':$( "input[name='csrfmiddlewaretoken']" ).val()}
	}).done(function(res) {
		if (res.status =='2'){
			col_row = ''
			$.each(res.columns, function( index, value){
				bg =$(table).css('background');
				value = value[0]
				if(index==0){
