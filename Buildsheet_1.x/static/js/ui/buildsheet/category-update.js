	...
	$('#l2_table').on('change', function(e) {
		...
		if ($("#l2_table option:selected").val()!='' /* && $("#l2_table option:selected").val()!='pd_activity'*/){
			...
			$.ajax({
				...
				$('.jschema_1,.jschema_2,.wschema').html(schema_select);
				loadMapping();
			});
...
function loadMapping() {
	...
	$('.load-build').on('click', function(e) {
		...
		if(validateBeforeSubmit()){
			buildAliasDetails();
			if($('#progressbar').find('li:eq(1)').hasClass('active')){
...
//Update functions
function loadMeta() {
	var meta_id =$(location).attr('href').match(/update\/\d+/g)[0].match(/\d+$/g);
...
function fillMeta(response) {
	...
	$('#l2_table').val(response.category).trigger("change").prop('readonly', 'true');
...
function fillSchema(schema,schema_attr,table){
	$(schema_attr).val(schema).trigger("change").prop('readonly', 'true');
...
function loadBuild() {
	...
	$.ajax({
		...
		if(res.status=='2'){
			$('#data-source').val(res.meta[20]).trigger('change');
			$('#data-source-version').val(res.meta[21]);
